...
NSString *const kHBNativeAdAdaptiveHeightKey = @"AdaptiveHeight";
...
UIEdgeInsets SafeAreaInsets_HBUnityNative() {
...
- (void)entryScenarioWithPlacementID:(NSString *)placementID scenarioID:(NSString *)scenarioID{
    ...
    [[HBAdManager sharedManager] entryNativeScenarioWithPlacementID:placementID scene:scenarioID];
}

-(NSString*) getValidAdCaches:(NSString *)placementID {
    NSArray *array = [[HBAdManager sharedManager] getNativeValidAdsForPlacementID:placementID];
...
-(void) showNativeAdWithPlacementID:(NSString*)placementID metricsJSONString:(NSString*)metricsJSONString extraJsonString:(NSString*)extraJsonString {
    if ([self isNativeAdReadyForPlacementID:placementID]) {
        ...
        NSLog(@"HBNativeAdWrapper::extraDict:%@",extraDict);
        ...
        if (extraDict[kHBNativeAdAdaptiveHeightKey] != nil) {
            configuration.sizeToFit = [extraDict[kHBNativeAdAdaptiveHeightKey] boolValue];
        }
...
//            [adview addSubview:label];
            ...
            UIEdgeInsets safeAreaInsets = SafeAreaInsets_HBUnityNative();
