...
-(NSString*) getValidAdCaches:(NSString *)placementID {
    NSArray *array = [[HBAdManager sharedManager] getRewardedVideoValidAdsForPlacementID:placementID];
...
- (void)entryScenarioWithPlacementID:(NSString *)placementID scenarioID:(NSString *)scenarioID{
    ...
    [[HBAdManager sharedManager] entryRewardedVideoScenarioWithPlacementID:placementID scene:scenarioID];
}
...
-(void) addAutoLoadAdPlacementID:(NSString*)placementID callback:(void(*)(const char*, const char*))callback {
    ...
    [[HBRewardedVideoAutoAdManager sharedInstance] addAutoLoadAdPlacementIDArray:placementIDArray];
...
-(void) removeAutoLoadAdPlacementID:(NSString*)placementID{
    ...
    [[HBRewardedVideoAutoAdManager sharedInstance] removeAutoLoadAdPlacementIDArray:placementIDArray];
}

-(BOOL) autoLoadRewardedVideoReadyForPlacementID:(NSString*)placementID {
    NSLog(@"Unity: autoLoadRewardedVideoReadyForPlacementID--%@--%d",placementID,[[HBRewardedVideoAutoAdManager sharedInstance] autoLoadRewardedVideoReadyForPlacementID:placementID]);
    return [[HBRewardedVideoAutoAdManager sharedInstance] autoLoadRewardedVideoReadyForPlacementID:placementID];
}

-(NSString*) getAutoValidAdCaches:(NSString *)placementID{
    NSArray *array = [[HBRewardedVideoAutoAdManager sharedInstance] checkValidAdCachesWithPlacementID:placementID];
    NSLog(@"Unity: getAutoValidAdCaches::array = %@", array);
    return array.jsonString;
}

-(NSString*) checkAutoAdStatus:(NSString *)placementID {
    HBCheckLoadModel *checkLoadModel = [[HBRewardedVideoAutoAdManager sharedInstance] checkRewardedVideoReadyAdInfo:placementID];
    NSMutableDictionary *statusDict = [NSMutableDictionary dictionary];
    statusDict[@"isLoading"] = @(checkLoadModel.isLoading);
    statusDict[@"isReady"] = @(checkLoadModel.isReady);
    statusDict[@"adInfo"] = checkLoadModel.adOfferInfo;
    NSLog(@"HBRewardedVideoWrapper::checkAutoAdStatus statusDict = %@", statusDict);
    return statusDict.jsonString;
    ...
    if ([customDataJSONString isKindOfClass:[NSString class]]) {
        ...
        if ([extraDict isKindOfClass:[NSDictionary class]]) {
            ...
            if (extraDict[kLoadExtraUserIDKey] != nil) {
                extra[kHBAdLoadingExtraUserIDKey] = extraDict[kLoadExtraUserIDKey];
            }
            if (extraDict[kLoadExtraMediaExtraKey] != nil) { extra[kHBAdLoadingExtraMediaExtraKey] = extraDict[kLoadExtraMediaExtraKey];
            }
        ...
        [[HBRewardedVideoAutoAdManager sharedInstance] setLocalExtra:extra placementID:placementID];
    }
}
...
-(void) entryAutoAdScenarioWithPlacementID:(NSString*)placementID scenarioID:(NSString*)scenarioID{
    ...
    [[HBRewardedVideoAutoAdManager sharedInstance] entryAdScenarioWithPlacementID:placementID scenarioID:scenarioID];
}
...
-(void) showAutoRewardedVideoWithPlacementID:(NSString*)placementID extraJsonString:(NSString*)extraJsonString {
    ...
    NSLog(@"HBRewardedVideoWrapper::showAutoRewardedVideoWithPlacementID = %@ extraJsonString = %@", placementID,extraJsonString);
    
    NSLog(@"HBRewardedVideoWrapper::extraDict = %@", extraDict);
    
    [[HBRewardedVideoAutoAdManager sharedInstance] showAutoLoadRewardedVideoWithPlacementID:placementID scene:extraDict[kHBUnityUtilitiesAdShowingExtraScenarioKey] inViewController:[UIApplication sharedApplication].delegate.window.rootViewController delegate:self];
}
