...
namespace HyperBid.Api
{
    public interface HBInterstitialAdListener
    {
		 ...
		 * @param placementId 广告位id
		 */ 
        void onInterstitialAdShow(string placementId, HBCallbackInfo callbackInfo);
		/***
		 ...
		 * @param placementId 广告位id
		 */ 
        void onInterstitialAdClose(string placementId, HBCallbackInfo callbackInfo);
		/***
		 * 广告点击（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 * @param placementId 广告位id
		 */ 
        void onInterstitialAdClick(string placementId, HBCallbackInfo callbackInfo);
        /**
        *广告开始播放视频（注意：对于Android来说，所有回调方法均不在Unity的主线程）
        * @param placementId 广告位id
        */
        void onInterstitialAdStartPlayingVideo(string placementId, HBCallbackInfo callbackInfo);
        /**
        *广告视频播放结束（注意：对于Android来说，所有回调方法均不在Unity的主线程）
        * @param placementId 广告位id
        */
        void onInterstitialAdEndPlayingVideo(string placementId, HBCallbackInfo callbackInfo);
        /**
